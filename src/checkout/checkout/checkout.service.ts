...
export class CheckoutService {
    ...
    async getcheckout(){
     ...
     async deletecheckout(req:checkoutDto){
        try{
         const delcheckout=await this.checkoutModel.deleteOne({checkoutId:req.checkoutId})
         if(delcheckout){
            return{
                statusCode:HttpStatus.OK,
     ...
     async editcheckout(req:checkoutDto){
        try{
            const resEdit=await this.checkoutModel.updateOne({checkoutId:req.checkoutId},{$set:{checkout:req.checkout }})
            if(resEdit){
                return{
                    statusCode:HttpStatus.OK,
     ...
     async  checkoutbyId(req:checkoutDto){
        try{
            const rescheckoutId=await this.checkoutModel.find({checkoutId:req.checkoutId})
            if(rescheckoutId){
                return{
                    statusCode:HttpStatus.OK,
